...
public function show($id)
{
    ...
    $product = Product::join('productstore', 'product.id', '=', 'productstore.product_id')
        ...
        ->join('category', 'product.category_id', '=', 'category.id')
        ->join('productsale', 'product.id', '=', 'productsale.product_id')// kết nối với bảng productimage có id bằng product_id
        ->select(
            'product.id',
            'product.name',
            'product.slug',
...
{
    $product = Product::withTrashed()->find($id); // Tìm sản phẩm kể cả đã bị soft delete
    ...
    $product->forceDelete(); // Sử dụng forceDelete để xóa vĩnh viễn
